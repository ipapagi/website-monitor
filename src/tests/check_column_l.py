#!/usr/bin/env python3
"""Έλεγχος της στήλης L (protocol_date) στο generated XLSX"""
...
if 'ΝΕΑ' in wb.sheetnames:
    ws = wb['ΝΕΑ']
    print("=" * 80)
    print("ΣΤΗΛΗ L (Ημ/νία Πρωτ) - ΝΕΑ ΑΙΤΗΣΕΙΣ")
    print("=" * 80)
    count = 0
    filled = 0
    for row in range(3, min(13, ws.max_row + 1)):  # Πρώτες 10 εγγραφές
        case_id = ws[f'B{row}'].value
        protocol_date = ws[f'L{row}'].value
        
        count += 1
        if protocol_date:
            filled += 1
        
        status = "✅" if protocol_date else "❌"
        print(f"{status} Σειρά {row}: Case_ID={case_id}, Ημ/νία Πρωτ={protocol_date}")
...
if 'ΠΑΛΙΑ' in wb.sheetnames:
    ws = wb['ΠΑΛΙΑ']
    print("\n" + "=" * 80)
    print("ΣΤΗΛΗ L (Ημ/νία Πρωτ) - ΠΑΛΙΕΣ ΑΙΤΗΣΕΙΣ")
    print("=" * 80)
    count = 0
    filled = 0
    for row in range(3, min(13, ws.max_row + 1)):  # Πρώτες 10 εγγραφές
