...
- ✅ JSON φιλικό για Power Automate/Teams (adaptive cards, post στο chat υποστήριξης)
- ✅ Εξαγωγή/ενημέρωση καταγραφής αιτήσεων σε Excel (νέες ή όλες)
- ✅ Emails ανά Διεύθυνση με attachments για νέες αιτήσεις (με προαιρετική αποστολή στο chat)
...
# Directory Emails - Email διευθύνσεων για νέες αιτήσεις (προαιρετικό)
...
# Προσθέστε περισσότερες διευθύνσεις με ίδιο μοτίβο: DIRECTORY_EMAIL_<DIRECTORY_NAME>=email

# Teams Webhook (προαιρετικό, για αποστολή στο chat υποστήριξης)
...
python src/main.py --send-directory-emails                # Emails ανά Διεύθυνση με attachments
...
| `--send-directory-emails` | Δημιουργεί και στέλνει emails ανά Διεύθυνση με attachments |
...
### Power Automate / Teams ροή
- Χρησιμοποίησε το `/sede/daily` (flat JSON) σαν trigger σε Flow για adaptive card.
- Το Flow μπορεί να δημοσιεύει σύνοψη στο group chat υποστήριξης και να επισυνάπτει το `.xlsx` export (`scope=new|all`).
- Έτσι ενημερώνεται αυτόματα το chat και το αρχείο καταγραφής αιτήσεων.
